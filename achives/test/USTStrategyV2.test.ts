...
import { BentoBoxV1, IExchangeRateFeeder, USTMock, USTStrategy, USTStrategyV2 } from "../typechain";
import { advanceTime, getBigNumber, impersonate } from "../utilities";

const degenBox = "0xd96f48665a1410C0cd669A88898ecA36B9Fc2cce";

let snapshotId;
let OldUSTStrategy: USTStrategy;
let NewUSTStrategy: USTStrategyV2;
...
describe("USTStrategyV2", async () => {
  before(async () => {
    ...
    await deployments.fixture(["USTStrategyV2"]);
    await impersonate(xMerlin);

    xMerlinSigner = await ethers.getSigner(xMerlin);

    OldUSTStrategy = await ethers.getContractAt<USTStrategy>("USTStrategy", "0xe6191aa754f9a881e0a73f2028edf324242f39e2");
    NewUSTStrategy = await ethers.getContract("USTStrategyV2");
    ...
    await expect(
      OldUSTStrategy.connect(xMerlinSigner).safeHarvest(ethers.constants.MaxUint256, true, ethers.constants.MaxUint256, false)
    ...
    await NewUSTStrategy.safeHarvest(ethers.constants.MaxUint256, true, ethers.constants.MaxUint256, false); // rebalances into the strategy
  ...
  it("should report a profit", async function () {
    ...
    // redeem profits
    await NewUSTStrategy.redeemEarnings();
    ...
    await NewUSTStrategy.safeHarvest(0, false, 0, false);
  ...
  it("should allow to update feeder", async () => {
    await NewUSTStrategy.updateExchangeRateFeeder("0xd96f48665a1410C0cd669A88898ecA36B9Fc2cce");
    expect(await NewUSTStrategy.feeder()).to.be.equal("0xd96f48665a1410C0cd669A88898ecA36B9Fc2cce");
  });
});
